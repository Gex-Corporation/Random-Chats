const firebaseConfig = {
        ...
        // ক্রিপ্টো, MQTT ও প্রেজেন্স স্টেট
        let client = null;
        ...
        const ALL_USERS_CACHE_TTL = 5 * 60 * 1000; // 5 মিনিট
        ...
        // --- ক্রিপ্টোগ্রাফি ইঞ্জিন (ECDH + AES-256 GCM) ---
        ...
        async function initOrLoadKeys(uid) {
            ...
            if (privStored && pubStored) {
                try {
                    ...
                    console.warn("ক্রিপ্টো চাবি রিকভারি ব্যর্থ, নতুন তৈরি হচ্ছে...", e);
                }
            }
        ...
        // --- ৪৮ ঘণ্টার ক্লিনআপ ফিল্টার ও লোকাল স্টোরেজ ---
        ...
        // --- Offline মেইলবক্স সিঙ্ক (ফায়ারবেস ব্যাকআপ উদ্ধার) ---
        async function syncOfflineMailbox() {
                ...
                if (doc.exists && doc.data().messages && doc.data().messages.length > 0) {
                    ...
                    for (const m of messages) {
                        ...
                        if (aesKey) {
                            ...
                            } catch (err) { console.error("মেইলবক্স ডিক্রিপশন ব্যর্থ:", err); }
                        }
                    }
            ...
            } catch (e) { console.error("মেইলবক্স সিঙ্ক ব্যর্থ:", e); }
        }
        ...
        authBtn.addEventListener('click', async () => {
            ...
            try {
                if (isSignupMode) {
                    ...
                    // ✅ FIX: Save basic info to the aggregated document for cheap reads
                    try {
        ...
        async function loginSuccess(uid) {
            ...
            unsubscribeProfile = db.collection('users').doc(uid).onSnapshot(async doc => {
                if (doc.exists) {
                    myProfile = doc.data();
                    // ✅ FIX: শুধুমাত্র publicKey সম্পূর্ণ absent হলেই write করো।
                    // JSON.stringify comparison কখনো কখনো property order-এর কারণে
                    // false দেয় এবং অনন্ত write→snapshot→write loop তৈরি করে।
        ...
        function logout() {
            ...
            if (unsubscribeGroups) unsubscribeGroups();
            if (client) {
                // তাৎক্ষণিক Offline স্ট্যাটাস পাবলিশ
            ...
            aesKeyCache.clear();
            userStatusMap.clear();
            // ✅ FIX: Logout-এ cache পরিষ্কার করো — নতুন session-এ fresh data পাবে
            allUsersCache = null;
            allUsersCacheTime = 0;
        ...
        function updateProfileUI() {
            ...
            document.getElementById('myCountryFlag').textContent = flags[myProfile.country] || '🌍';
        ...
        function updatePresenceUI(targetUid, status) {
            ...
            // ৩. চ্যাট হেডার আপডেট (যদি চ্যাট ওপেন থাকে)
        ...
        async function loadAllUsers() {
            ...
            try {
                // ✅ FIX: Cache — ৫ মিনিটের মধ্যে আবার call হলে Firebase hit করবে না
                const now = Date.now();
                let usersList = [];
        ...
        // --- MQTT লাইভ ইঞ্জিন ও LWT প্রেজেন্স আর্কিটেকচার ---
        function setupMQTT() {
            const myTopic = 'ghostchat/user/' + currentUserId;
            const myStatusTopic = 'ghostchat/status/' + currentUserId;

            // LWT (Last Will and Testament): সংযোগ বিচ্ছিন্ন হওয়া মাত্র ব্রোকার নিজে থেকেই 'offline' পাঠাবে
            ...
            client.on('connect', () => {
                ...
                client.subscribe('ghostchat/status/+'); // সব ফ্রেন্ডের স্ট্যাটাস ট্র্যাক
                myGroups.forEach(g => client.subscribe('ghostchat/group/' + g.id));

                // Onlineে প্রবেশ করার সাথে সাথে retain: true সহ 'online' সিগন্যাল প্রচার
                client.publish(myStatusTopic, 'online', { qos: 1, retain: true });
            });

            client.on('message', async (topic, message) => {
                try {
                    // ১. Online / Offline প্রেজেন্স হ্যান্ডলার
                    if (topic.startsWith('ghostchat/status/')) {
                        const senderUid = topic.replace('ghostchat/status/', '');
                        const status = message.toString().trim(); // 'online' অথবা 'offline'
                        updatePresenceUI(senderUid, status);
                        return;
                    }

                    // ২. মেসেজ ও ACK হ্যান্ডলিং
                    const data = JSON.parse(message.toString());

                    // ACK প্রাপ্তি (মেসেজ অপর প্রান্তে পৌঁছানোর কনফার্মেশন)
                    ...
                    // SEEN প্রাপ্তি (মেসেজ অপর প্রান্তে দেখা হয়েছে)
                    ...
                    if (data.type === 'live_chat' && data.targetUid === currentUserId) {
                        // তাৎক্ষণিক ACK রিটার্ন (প্রেরকের টাইমার বন্ধ করা)
                        ...
                        if (aesKey) {
                            try {
                                ...
                                console.error("ডিক্রিপশন ত্রুটি:", decryptErr);
                                aesKeyCache.delete(data.sender);
                            }
                        }
                    }
                } catch (err) { console.error("MQTT ইভেন্ট ত্রুটি:", err); }
            });
        }
            ...
            if (msg) {
                ...
                if (activeFriendUid === friendUid) {
                    ...
                    if (el) {
                        ...
                        el.textContent = newStatus === 'read' ? '✓✓' : (newStatus === 'delivered' ? '✓' : '☁️');
                    }
                }
            }
        }
        ...
        window.openPrivateChat = async (friendUid, friendName, friendGender) => {
            ...
            try {
                ...
                console.error("এনক্রিপশন প্রস্তুতি ত্রুটি:", err);
        ...
        window.openGroupChat = async (g) => {
            ...
            document.getElementById('chatPartnerAvatar').firstChild.textContent = '👥';
        ...
        function appendMessageBubble({ id, text, sender, timestamp, status, senderName }) {
            ...
            let markHtml = '';
            if (sender === 'me') {
                const markChar = status === 'read' ? '✓✓' : (status === 'delivered' ? '✓' : (status === 'mailbox' ? '☁️' : '⏳'));
        ...
        // --- মেসেজ সেন্ড ও স্মার্ট ACK টাইমআউট ---
        async function sendMsg() {
            ...
            if (activeGroupId) {
                ...
                // ✅ FIX: Group Chat এখন 100% রিয়েল-টাইম এবং Ephemeral।
                // অফলাইন ইউজারদের জন্য ফায়ারবেসে কোনো Write হবে না।
            ...
            const timer = setTimeout(async () => {
                ...
                try {
                    ...
                    console.error("মেইলবক্স ব্যাকআপ ব্যর্থ:", e);
                }
            }, 2000);
        ...
        // ============================================================
        // ========== Live TV Chat ফিচার (Firebase Live Collection) ===
        // ============================================================

        // Firebase থেকে লাইভ চ্যানেল লোড করে রেন্ডার করবে
        async function loadLiveChannels() {
            const container = document.getElementById('liveChannelsList');
            if (!container) return;

            // ✅ FIX: Session cache — একবার load হলে আর Firebase hit করবে না
            ...
            try {
                ...
                liveChannelsCache = channels; // cache করে রাখো
        ...
        // Live channel কার্ড রেন্ডার করার helper
        ...
        // XSS প্রতিরোধের জন্য HTML escape
        function escapeHtml(str) {
            if (!str) return '';
            return str.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#39;');
        }

        // লাইভ চ্যাট লিংক নতুন ট্যাবে ওপেন করবে
        window.openLiveChat = function (channelName, chatUrl) {
            window.open(chatUrl, '_blank', 'noopener,noreferrer');
        };

        // লাইভ ভিউ থেকে ড্যাশবোর্ডে ফিরে যাবে এবং iframe src পরিষ্কার করবে
        window.closeLiveView = function () {
            // iframe src পরিষ্কার করা গুরুত্বপূর্ণ যাতে ব্যাকগ্রাউন্ডে চলতে না থাকে
...
const i18n = {
    ...
    bn: {
        friends_sub: "বন্ধুরা",
        requests_sub: "অনুরোধ",
        search_sub: "সার্চ করুন",
        create_group: "+ গ্রুপ তৈরি করুন",
        empty_friends: "আপনার বন্ধু তালিকা খালি",
        empty_requests: "কোনো ফ্রেন্ড রিকোয়েস্ট নেই",
        search_placeholder: "ইউজার আইডি বা নাম লিখুন...",
        search_btn: "সার্চ",
        search_hint: "ইউজার আইডি বা নাম দিয়ে খুঁজুন",
        nav_random: "র‍্যান্ডম ইউজার",
        nav_chats: "চ্যাটস",
        nav_friends: "বন্ধুরা",
        nav_live: "লাইভ",
        nav_settings: "সেটিংস",
        ...
        language_label: "ভাষা",
        chat_developer: "ডেভেলপারের সাথে কথা বলুন",
        about_app: "অ্যাপটি সম্পর্কে",
        dev_name: "ডেভেলপার",
        dev_status: "🟢 সাপোর্ট এবং ফিডব্যাক",
        dev_chat_hint: "ডেভেলপারকে আপনার ফিডব্যাক পাঠান...",
        chat_placeholder: "মেসেজ লিখুন..."
    }
};
...
function openDeveloperChat() {
    ...
    stream.innerHTML = '<div class="empty-placeholder" data-i18n="dev_chat_hint">' + (document.getElementById('languageSelect').value === 'bn' ? 'ডেভেলপারকে আপনার ফিডব্যাক পাঠান...' : 'Send your feedback to the developer...') + '</div>';
